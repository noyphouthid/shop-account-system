...
function CustomerContent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data } = await supabase.from('orders').select(`*, customer_payments(amount)`)
  ...
  const filteredData = orders.filter(o => {
    ...
    const hasDebt = (o.total_price - paid) > 0;
    ...
    return hasDebt && matchesDate && matchesSearch
  })

  const totalDebt = filteredData.reduce((sum, o) => {
    const paid = (o.initial_deposit || 0) + (o.customer_payments?.reduce((s:any, p:any) => s + p.amount, 0) || 0);
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4">
            <div className="p-4 bg-rose-50 text-rose-500 rounded-2xl"><Users size={32}/></div>
            <div>
              <h1 className="text-3xl font-black text-slate-800">ຍອດຄ້າງຮັບຈາກລູກຄ້າ</h1>
          ...
          <div className="bg-rose-50 px-8 py-4 rounded-[24px] text-center border border-rose-100">
            <p className="text-rose-500 text-xs font-black uppercase mb-1">ຍອດຄ້າງຮັບລວມ</p>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100 text-[11px] font-black text-slate-400 uppercase tracking-widest">
                ...
                <th className="px-8 py-5 text-right">ລາຄາເຕັມ</th>
                <th className="px-8 py-5 text-right">ຈ່າຍແລ້ວ</th>
                <th className="px-8 py-5 text-right text-rose-500 bg-rose-50/30">ຍັງຄ້າງ</th>
              </tr>
            </thead>
              ...
              {filteredData.map(o => {
                ...
                return (
                  <tr key={o.id} className="hover:bg-slate-50/80 transition-colors">
                    <td className="px-8 py-5 font-black text-slate-900">{o.order_code}</td>
                    <td className="px-8 py-5 text-right">{o.total_price.toLocaleString()}</td>
                    <td className="px-8 py-5 text-right text-emerald-600">{paid.toLocaleString()}</td>
                    <td className="px-8 py-5 text-right text-rose-600 font-black bg-rose-50/10">{(o.total_price - paid).toLocaleString()}</td>
                  </tr>
                )
              })}
            </tbody>
...
export default function CustomerDebt() {
  return <Suspense fallback={<div>Loading...</div>}><CustomerContent /></Suspense>
}